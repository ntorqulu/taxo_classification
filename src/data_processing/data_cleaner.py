...
class TaxonomyDataCleaner:
    ...
    def clean_data(
        ...
        if enforce_taxonomy_consistency:
            ...
            if verbose and inconsistencies > 0:
                print(f"Total taxonomic inconsistencies: {inconsistencies}")
        
        # 7. Filter by taxonomic completeness
        if require_complete_ranks_up_to:
            rank_indices = {rank.replace('_name', ''): i for i, rank in enumerate(
                [r.replace('_name', '') for r in self.taxonomy_ranks]
            )}
    ...
    def balance_dataset(self, df, target_column, method='undersample', max_ratio=100, min_samples=5, 
        ...
        # Check if there are too few samples for some methods
        min_class_count = counts.min()
        if min_class_count < min_samples and method in ['smote', 'adasyn', 'smoteenn', 'smotetomek']:
            if verbose:
                print(f"Warning: Some classes have fewer than {min_samples} samples, which is too few for {method}.")
                print(f"Falling back to 'random_oversample' method.")
